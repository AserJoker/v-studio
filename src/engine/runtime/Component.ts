...
export class Component<T = any, C extends Record<string, unknown> = {}> implements IComponent<T, C>{
    ...
    public constructor(state: T = {} as T, computed?: IComputed<T, C>, key?: string) {
    ...
    public on<T = unknown>(type: string, cb: (arg: T, key?: string | undefined) => void): () => void {
        return Component.$listener.on(type, cb);
    }
    public off<T = unknown>(type: string, cb: (arg: T, key?: string | undefined) => void): void {
        return Component.$listener.off(type, cb);
    }
    public once<T = unknown>(type: string, cb: (arg: T, key?: string | undefined) => void): void {
        return Component.$listener.once(type, cb);
    }
    public emit<T = unknown>(type: string, arg?: T | undefined, key?: string | undefined): void {
        return Component.$listener.emit(type, arg, key ?? this.key);
    }
    public field<K extends keyof T, CC extends Record<string, unknown>>(field: K, computed?: IComputed<T[K], CC>, key?: string): IComponent<T[K], CC> {
    ...
    public watch(key: string | RegExp, callback: () => void | Promise<void>) {
        const watcher: Watcher = { key: typeof key === 'string' ? key.replace('\]', '').replace('\[', '.') : key, callback };
    ...
    public render(schema: ISchemaNode, slot?: string): JSX.Element | string | null {
        if (slot) {
            ...
            return React.createElement(React.Fragment, {}, slots.map(slot => {
                let $comp: IComponent;
                if (slot.field) {
                    $comp = cache[slot.field] ?? this.field(slot.field as keyof T, undefined, slot.key);
